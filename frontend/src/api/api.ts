import type { Category, Referral, ReferralFormValues, Resource, ResourceFormValues } from '../types'

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL ?? 'http://localhost:8000'

export async function listResources(_params?: { q?: string; category?: Category | 'All' }): Promise<Resource[]> {
  void _params
  void API_BASE_URL
...
export async function createResource(_payload: ResourceFormValues): Promise<Resource> {
  void _payload
  void API_BASE_URL
...
export async function listResourceReferrals(_resourceId: number): Promise<Referral[]> {
  void _resourceId
  void API_BASE_URL
...
export async function createReferral(_payload: ReferralFormValues): Promise<Referral> {
  void _payload
  void API_BASE_URL
